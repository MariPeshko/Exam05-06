...
#define _POSIX_C_SOURCE 200809L // This macro tells the C library that you want to use features compliant with the POSIX.1-2008 standard
 ...
 */
void handle_new_connection(int sockfd, int *fd_count,
 ...
 */
void handle_client_data(int sockfd, int *fd_count,
		...
		struct pollfd **pfds)
{
	for(int i = 0; i < *fd_count; i++) {
		...
		if ((*pfds)[i].revents & (POLLIN | POLLHUP)) {
			...
			if ((*pfds)[i].fd == sockfd) {
				// If we're the listener sockfd, it's a new connection
				handle_new_connection(sockfd, fd_count, fd_size,
						pfds);
			} else {
...
/*
 * Main: create a listener `sockfd` and connection set, loop forever
...
int	main(void)
{
	...
	// Add sockfd (listener) to the set;
