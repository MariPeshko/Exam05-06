...
void	handle_new_connection(int sockfd, std::vector<pollfd>& pfds) {
	...
	newfd = accept(sockfd, reinterpret_cast<struct sockaddr*>(&remoteaddr), &addrlen);
...
void	handle_client_data(int sockfd, std::vector<pollfd>& pfds, size_t& i) {
...
void	process_connections(int sockfd, std::vector<pollfd>& pfds) {
    for (size_t i = 0; i < pfds.size(); ++i) {
        ...
        if (pfds[i].revents & (POLLIN | POLLHUP)) {
            ...
            if (pfds[i].fd == sockfd) {
                ...
                handle_new_connection(sockfd, pfds);
            } else {
                // Otherwise we're just a regular client
                handle_client_data(sockfd, pfds, i);
            }
        }
    }
}
...
int	main() {
	...
	while (g_running) {
		...
		if (poll_count > 0) {
			...
			process_connections(sockfd, pfds);
		}
	}
